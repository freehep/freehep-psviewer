...
public class ControlOperator extends PSOperator {
    ...
    public static Class[] operators = {
        Exec.class, If.class, IfElse.class, 
        For.class, Repeat.class, Loop.class,
        Exit.class, Stop.class, Stopped.class,
        CountExecStack.class, ExecStack.class,
        Quit.class, Start.class, 
	    EExec.class
    };
...
class For extends ControlOperator implements LoopingContext {
    ...
    public boolean execute(OperandStack os) {
        ...
        if (procedure == null) {
            ...
            os.execStack().pop();
            if ((limit instanceof PSReal) ||
                (inc instanceof PSReal) || 
...
class Exit extends ControlOperator {
    ...
    public boolean execute(OperandStack os) {
        ...
        if (o == null) {
            System.err.println("Error: No enclosing LoopingContext; Command: exit");
            os.execStack().push("quit");
            os.execStack().push(os.dictStack().errorDictionary().get("handleerror"));
            return false;
        } 
...
class Stop extends ControlOperator {
    ...
    // Our stop operator will print the error message if not in a stopped context
    public boolean execute(OperandStack os) {
...
class EExec extends ControlOperator {
	...
	if (os.checkType(PSDataSource.class)) {
		...
		os.execStack().pop();
		os.execStack().push(new PSInputFile(new EEXECDecryption(source.getInputStream(),
									EEXECDecryption.EEXEC_R,
									EEXECDecryption.N), source.getDSC()));
	    } catch (IOException e) {
