...
public class FileOperator extends PSOperator {
    ...
    public static Class[] operators = {
        FileFile.class, Filter.class, CloseFile.class,
        Read.class, Write.class,
        ReadHexString.class, WriteHexString.class,
        ReadString.class, WriteString.class,
        ReadLine.class, BytesAvailable.class,
        Flush.class, FlushFile.class, ResetFile.class,
        Status.class, Run.class, CurrentFile.class,
        DeleteFile.class, RenameFile.class,
        FilenameForAll.class, SetFilePosition.class, FilePosition.class,
        Print.class, FileEqual.class, FileEqualEqual.class,
        Stack.class, PStack.class
    };
    ...
    public boolean execute(OperandStack os) {
        ...
        try {
            if (filename.equals("%stdin")) {
                ...
                boolean append = (access.length() > 0) && (access.charAt(0) == 'a');
...
class Filter extends FileOperator {
    ...
    public boolean execute(OperandStack os) {
        ...
        try {
            return filterName.endsWith("Encode") ? encode(os, filterName) : decode(os, filterName);
...
class ReadHexString extends FileOperator {
    ...
    public boolean execute(OperandStack os) {
        ...
        if (string.size() <= 0) {
            ...
            try {
                InputStream in = new ASCIIHexInputStream(((PSInputFile)file).getInputStream(), true);
...
class WriteHexString extends FileOperator {
    ...
    public boolean execute(OperandStack os) {
        ...
        PSFile file = os.popFile();
        try {
            if (!os.isSecure()) throw new IOException();
            OutputStream out = new ASCIIHexOutputStream(((PSOutputFile)file).getOutputStream());
            for (int i=0; i<string.size(); i++) {
